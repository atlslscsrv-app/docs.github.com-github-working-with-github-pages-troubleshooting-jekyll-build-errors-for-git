...
const productionDependencies = deps.getProductionDependencies(
...
function packageTask(
	...
	return () => {
		...
		const extensions = gulp.src(".build/extensions/**", {
			base: ".build",
			dot: true,
		});
		...
		if (shouldSetupSettingsSearch()) {
			productJsonUpdate.settingsSearchBuildId = getSettingsSearchBuildId(
				packageJson
			);
		}
		...
		const api = gulp
			.src("src/vs/vscode.d.ts")
			.pipe(rename("out/vs/vscode.d.ts"));
		...
		const dependenciesSrc = _.flatten(
		...
		let result = all
			...
			.pipe(
				filter(["**", "!LICENSE", "!LICENSES.chromium.html", "!version"], {
					dot: true,
				})
			);
		...
		if (opts.stats) {
			result.on("end", () => {
				...
				submitAllStats(product, commit).then(() =>
					console.log("Submitted bundle stats!")
				);
			});
		}
	...
	return es.merge(
		[...i18n.defaultLanguages, ...i18n.extraLanguages].map((language) => {
			...
			return i18n
				...
				.pipe(vfs.dest(`../vscode-translations-import/${language.id}/setup`));
		})
	);
});
...
gulp.task("vscode-translations-import", function () {
	...
	return es.merge(
		[...i18n.defaultLanguages, ...i18n.extraLanguages].map((language) => {
			...
			return gulp
				.src(`${options.location}/${id}/setup/*/*.xlf`)
...
const generateVSCodeConfigurationTask = task.define(
...
gulp.task(
	task.define(
		...
		task.series(generateVSCodeConfigurationTask, () => {
			...
			return gulp.src(allConfigDetailsPath).pipe(
				azure.upload({
...
function shouldSetupSettingsSearch() {
	const branch = process.env.BUILD_SOURCEBRANCH;
	return (
		branch && (/\/master$/.test(branch) || branch.indexOf("/release/") >= 0)
	);
}
...
function getSettingsSearchBuildId(packageJson) {
	try {
		const branch = process.env.BUILD_SOURCEBRANCH;
		const branchId =
			branch.indexOf("/release/") >= 0 ? 0 : /\/master$/.test(branch) ? 1 : 2; // Some unexpected branch
